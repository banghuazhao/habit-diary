...
class InsightsViewModel {
    ...
    @ObservationIgnored
    @Dependency(\.insightsService) private var insightsService
    
    @CasePathable
    enum Route {
        case scoreBreakdownDetail(RatingBreakdownViewModel)
    ...
    func loadRatingData() async {
        ...
        scoreBreakdown = insightsService.calculateHabitScore()
        scoreBreakdownItems = insightsService.getScoreBreakdown()
    ...
    func onTapScoreBreakdownItem(_ item: ScoreBreakdownItem) {
        let detailViewModel = RatingBreakdownViewModel(category: item.category)
