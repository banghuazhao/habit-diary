...
struct HabitPageView: View {
    ...
    var body: some View {
                    ...
                    VStack(alignment: .leading, spacing: AppSpacing.small) {
                        ...
                        VStack(spacing: AppSpacing.small) {
                            ForEach(viewModel.habitAchievements, id: \.id) { achievement in
                                HabitBadgeRowView(achievement: achievement)
                                    .onTapGesture {
        ...
        .alert(
            ...
            Text("This will permanently delete \(viewModel.habit.truncatedName) and all its check-in history. This action cannot be undone. Are you sure you want to proceed?")
        }
    }
