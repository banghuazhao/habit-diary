...
class JournalHomeViewModel {
    var todayHabits: [JournalHabit] {
        updateTodayHabits()
    }

    var selectedDate: Date = Date()

    struct PendingReflectionContext: Identifiable {
        let id = UUID()
    ...
    enum Route {
        ...
        case addNote(PendingReflectionContext)
    }

    var route: Route?

    var isEditing: Bool = false
    var currentQuote: DailyReflection?
    ...
    @ObservationIgnored
    @FetchAll(DiaryEntry.all, animation: .default)
    var checkIns
    ...
    @ObservationIgnored
    @Dependency(\.reminderNotificationCenter) private var reminderNotificationCenter
    @ObservationIgnored
    @Dependency(\.defaultDatabase) private var database
    @ObservationIgnored
    @Dependency(\.dailyReflectionService) private var dailyReflectionService
    ...
    func updateMotivationalQuote() {
        withAnimation {
            if dailyReflectionService.shouldShowQuote() {
                currentQuote = dailyReflectionService.getRandomQuote()
    ...
    func dismissMotivationalQuote() {
        withAnimation {
            dailyReflectionService.dismissQuoteForToday()
            showMotivationalQuote = false
        }
    }

    private func updateTodayHabits() -> [JournalHabit] {
        habits
            .compactMap { habit -> JournalHabit? in
    ...
    func onTapHabitItem(_ todayHabit: JournalHabit) {
        ...
        withErrorReporting {
            if todayHabit.isCompleted {
                ...
                var savedCheckInRef: DiaryEntry?
                try dataBase.write { [selectedDate] db in
                    let draft = DiaryEntry.Draft(date: selectedDate, habitID: todayHabit.habit.id)
                    savedCheckInRef = try DiaryEntry.upsert { draft }.returning(\.self).fetchOne(db)
                ...
                if let savedCheckIn = savedCheckInRef {
                    route = .addNote(PendingReflectionContext(checkIn: savedCheckIn, habit: todayHabit.habit))
                }
            }
        }
    }

    func saveNoteForCheckIn(_ checkIn: DiaryEntry, note: String) {
        let trimmed = note.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        withErrorReporting {
            try dataBase.write { db in
                try db.execute(
                    sql: #"UPDATE "diaryEntries" SET "note" = ? WHERE "id" = ?"#,
    ...
    private func calculateStreakForFixedDays(habit: Habit, days: Set<Int>, unit: Calendar.Component, checkIns: [DiaryEntry]) -> Int {
        var streak = 0
    ...
    private func calculateStreakForNDaysPerPeriod(habit: Habit, checkIns: [DiaryEntry]) -> Int {
        var streak = 0
