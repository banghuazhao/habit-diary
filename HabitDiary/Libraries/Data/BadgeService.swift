...
class BadgeService {
    ...
    private func initializeAchievements() async {
        await withErrorReporting {
            try await database.write { db in
                let existingCount = try Badge.all.fetchCount(db)
                if existingCount == 0 {
                    // Insert all achievement definitions
                    for achievementDraft in BadgeDefinitions.all {
                        try Badge.upsert { achievementDraft }.execute(db)
                    }
                }
            }
        }
    ...
    func checkAchievementsAndShow(for checkIn: DiaryEntry) async {
            ...
            for achievement in allAchievements where !achievement.isUnlocked {
                if await checkAchievementCriteria(achievement, for: checkIn) {
                    let updatedAchievement = try await database.write { db in
                        ...
                        return try Badge.update(updatedAchievement).returning(\.self).fetchOne(db)
                    }
    ...
    private func checkAchievementCriteria(_ achievement: Badge, for checkIn: DiaryEntry) async -> Bool {
        // Decode criteria from string
        guard let criteria = BadgeCriteria.decode(from: achievement.criteria.encode()) else {
            return false
        }
    ...
    private func checkStreakAchievement(_ achievement: Badge, criteria: BadgeCriteria, for checkIn: DiaryEntry) async -> Bool {
    ...
    private func checkTotalCheckInsAchievement(_ achievement: Badge, criteria: BadgeCriteria, for checkIn: DiaryEntry) async -> Bool {
    ...
    private func checkPerfectWeekAchievement(_ achievement: Badge, criteria: BadgeCriteria, for checkIn: DiaryEntry) async -> Bool {
    ...
    private func checkPerfectMonthAchievement(_ achievement: Badge, criteria: BadgeCriteria, for checkIn: DiaryEntry) async -> Bool {
    ...
    private func checkEarlyBirdAchievement(_ achievement: Badge, criteria: BadgeCriteria, for checkIn: DiaryEntry) async -> Bool {
        let hour = userCalendar.component(.hour, from: checkIn.date)
        return hour < 8
    }
    
    private func checkNightOwlAchievement(_ achievement: Badge, criteria: BadgeCriteria, for checkIn: DiaryEntry) async -> Bool {
        let hour = userCalendar.component(.hour, from: checkIn.date)
        return hour >= 22
    }
    
    private func checkConsistencyAchievement(_ achievement: Badge, criteria: BadgeCriteria, for checkIn: DiaryEntry) async -> Bool {
    ...
    private func checkMilestoneAchievement(_ achievement: Badge, criteria: BadgeCriteria, for checkIn: DiaryEntry) async -> Bool {
...
private enum BadgeServiceKey: DependencyKey {
    static let liveValue = BadgeService()
}
